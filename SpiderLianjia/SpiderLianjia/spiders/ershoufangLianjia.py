...
class ershoufangLianjia(scrapy.Spider):
    ...
    def parse_house_page_res(self, response):
        ...
        if response.xpath('//html').re(r'houseId.*,'):
            ...
            yield scrapy.Request(history_price_query, callback=self.parse_history_price_page_res,
                                 meta={'house_item': item}, dont_filter=True)
        else:
            # 获得发布时间的月份
    ...
    def parse_house_page_com(self, response):
        ...
        if communityCode_query:
            ...
            yield scrapy.Request(history_price_query, callback=self.parse_history_price_page_com,
                                 meta={'house_item': item}, dont_filter=True)
        else:
            # 获得发布时间的月份
            time_list = time.localtime()
